...
public class Loader {
	...
	private static List<Integer> alBuffers = new ArrayList<Integer>();
...
/**
	...
	public static void cleanUp()
	{
		...
		for (Integer alBuffer : alBuffers)
		{
...
/**
	...
	public static ITexture loadTexture(ResourceLocation location)
	{
		...
		{
			LOGGER.warn("Could not find image at \'" + location + "\'");
...
/**
	...
	public static ITexture loadFontTexture(ResourceLocation location)
	{
		...
		{
			LOGGER.warn("Could not load image at \'" + location + "\'", e);
			return Loader.loadTexture(LoadingUtils.createMissingImage(256, 256));
		}
	}
...
/**
	 ...
	 * @return The sound created or null if it could not be loaded
	 * @throws UnsupportedAudioFileException
	 *             If the format for the file is Big-Endian
	 *//*

	@Nullable
	public static Sound loadSound(ResourceLocation location)
	{
		try
		{
			return loadSound(AudioSystem.getAudioInputStream(location.getInputStream()));
		}
		catch (Exception e)
		{
			LOGGER.warn("Could not load sound at \'" + location + "\'", e);
			return null;
		}
	}
	...
	*/
/**
	 * Loads a sound into memory from an input stream.
	 * 
	 * @param stream
	...
	@Nullable
	public static Sound loadSound(AudioInputStream stream) throws UnsupportedAudioFileException
	{
		...
		try
		{
			AL10.alBufferData(buffer, openALFormat, Buffers.storeDataInByteBuffer(IOUtils.toByteArray(stream)), (int) format.getSampleRate());
			return new SoundImpl(buffer, (long) (1000f * stream.getFrameLength() / format.getFrameRate()));
		}
		catch (Exception e)
		{
			e.printStackTrace();
